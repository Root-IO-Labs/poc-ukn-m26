...
- **wolfCrypt JNI (master)** - JCE provider (wolfJCE) with JNI bindings to wolfSSL FIPS
- **wolfSSL JNI v1.13.0** - JSSE provider (wolfJSSE) with JNI bindings to wolfSSL FIPS (Java 11 compatible)
- **Ubuntu 22.04 Jammy** - Container base (Eclipse Temurin official image)

> **Note:** Java 11 uses wolfSSL JNI v1.13.0-stable for Java 11 compatibility. v1.13.0 adds Java 17 support and works correctly with Java 11. The wolfCrypt JNI master branch is used as it doesn't have Java version dependencies and supports modern wolfSSL headers.
...
|----------|---------|-------------|
| `JAVA_HOME` | `/usr/local/openjdk-11` | Java installation directory |
